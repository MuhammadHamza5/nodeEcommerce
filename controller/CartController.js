...
//       const price = item.product.descount_price > 0
...
const getCart = async (request, response) => {
  try {
    ...
    for (const cartItem of cartData) {
      const cartAttributes = await cartAttributeModel
        .find({ cart: cartItem._id })
        .populate("product_attribute");

      const shopId = cartItem.shop._id.toString();

      if (!groupedCart[shopId]) {
        const seller = await sellerDataModel.findById(shopId).lean();
        if (!seller) continue;
      ...
      groupedCart[shopId].cart_data.push({
        ...cartItem.toObject(),
